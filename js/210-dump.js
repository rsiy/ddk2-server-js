...
// Version: 1.1
DDK.dump = function(object, showTypes) {
	var dump = '';
	...
	return DDK.escape.brackets("<script>" + script + "</script>" + dump);
	//winName.focus();  
	function recurse(o, type) {
		...
		switch (type) {		
			...
			case 'function':
				...
				r += '<table' + _dumpStyles(t,'table') + '><tr><th colspan="2"' + _dumpStyles(t,'th') + '>' + t + '</th></tr>';
				r += '<tr><td colspan="2"' + _dumpStyles(t,'td-value') + '><table' + _dumpStyles('arguments','table') + '><tr><td' + _dumpStyles('arguments','td-key') + '><i>Arguments: </i></td><td' + _dumpStyles(type,'td-value') + '>' + args + '</td></tr><tr><td' + _dumpStyles('arguments','td-key') + '><i>Function: </i></td><td' + _dumpStyles(type,'td-value') + '>'+ HtmlEncode(o)  + '</td></tr></table>';  
				j++;
				break;
			case 'domelement':
				var t = type;
		...
		case 'object':
			switch (use) {
				case 'table':  
					...
					break;
				case 'th':
					r = ' style="' + th + 'background-color:#0C59AB;"' + thScript;
					break;
				case 'td-key':
